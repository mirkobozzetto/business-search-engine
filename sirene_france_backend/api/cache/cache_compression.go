...
func (r *RedisCache) smartCompress(key string, jsonData []byte) (*CompressionResult, error) {
	...
	if originalSize <= COMPRESSION_THRESHOLD {
		return &CompressionResult{
			Data: jsonData, Key: key, IsCompressed: false,
			OriginalSize: originalSize, FinalSize: originalSize,
		}, nil
	}
	...
	if compressedSize > MAX_COMPRESSED_SIZE {
		return nil, fmt.Errorf("dataset too large after compression: %d MB", compressedSize/1024/1024)
	}
	...
	return &CompressionResult{
		Data: compressed, Key: COMPRESSION_PREFIX + key, IsCompressed: true,
		OriginalSize: originalSize, FinalSize: compressedSize, CompressionRatio: compressionRatio,
	}, nil
}
